...
test('it requires an id prop', async () => {
  console.error = jest.fn();

  const props = { id: null };
  render(BaseRadioInputGroup, { props });
...
test('it handles an array of number/string options', () => {
  ...
  const utils = render(BaseRadioInputGroup, { props });
  ...
  fireEvent.click(b);
  expect(utils.emitted().change[2][0]).toEqual(null);
});
...
// array of objects
test('it handles an array of object options and emits object', () => {
  const props = {
    id: 'some-id',
    value: null,
  ...
  fireEvent.click(a);
  expect(utils.emitted().change[1][0]).toEqual(null);
});

test('it handles an array of object options and emits key', () => {
  const props = {
    id: 'some-id',
    value: null,
    options: [{ id: 'a' }, { id: 'b' }, { id: 'c' }],
...
test('it accepts an key as value', () => {
  ...
  fireEvent.click(b);
  expect(utils.emitted().change[0][0]).toEqual(20);
});
